...
class ArrayType extends ComplexType
{
	...
	protected function implementArrayAccess()
	{
		...
		$offsetSetDock->addParam(
			PhpDocElementFactory::getParam('mixed', 'offset', 'The offset to assign the value to')
		);
	...
	protected function implementIterator()
	{
		...
		$keyDock->setReturn(
			PhpDocElementFactory::getReturn('string|null', 'Return the key of the current element or null')
		);
		$key = new PhpFunction(
			'public',
			'key',
		...
		$validDock->setReturn(
			PhpDocElementFactory::getReturn('boolean', 'Return the validity of the current position')
		);
